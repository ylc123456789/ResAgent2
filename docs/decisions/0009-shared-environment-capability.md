...
- 三个共享 Tool（capabilities）：`prepare_environment` / `run_setup` / `audit_env`。`run_setup` 允许 `pip install -r` / `pip install -e .` / `uv sync` / `poetry install` / `conda env update -f environment.yml`；禁止 `sudo`、`conda create/remove`、指定其它 `--prefix/-p/--name/-n`。`audit_env` 只证明基础环境正确（sys.executable / sys.prefix / Python 版本 / pip 可用 / prefix 匹配），不硬编码 torch。
