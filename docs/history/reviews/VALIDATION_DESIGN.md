...
状态：阶段 0 已完成源码盘点；阶段 1 本地与服务器验收完成，原始证据已独立复核通过。阶段 2 已完成实现与服务器验收，schema 17.0，原始证据已独立复核；阶段 3 暂缓实现（2026-09-26 决定），不作为阶段 1、2 收尾的前置条件。
...
### 阶段 3：新的运行前 Validation（暂缓）

2026-09-26 决定：暂缓实现额外的统一运行前 Validation，不为此新增阻断状态、自动修复流程或调整 Interpreter 的错误处理。阶段 1、2 已独立完成验收；阶段 3 不作为本轮收尾或后续合并评审的必要条件。

现有 Contracts 请求校验、Scheduler 依赖/预算/输入绑定检查、Runtime 工具参数/权限守卫、接收端登记校验及读取时冻结哈希检查继续生效。当前未发现必须依赖新增全量输入预检才能解决的正确性问题；其已确认收益主要是提前发现不可用输入，而重复 IO、拒绝后的状态推进和失败反馈会增加复杂度。统一应保持事实规则和职责一致，不要求每层新增同名 Validator。

后续若真实任务暴露坏输入导致明显无效执行，或出现既有边界无法覆盖的错误接受，再根据具体证据评估最小修复范围；需同时明确拒绝、记录、反馈和恢复语义。以下保留原候选范围供重新评估，不代表当前实施承诺：
...
若后续重新启动本阶段，应在实现前明确检查失败后不调用本次执行 Agent，以及对应的失败记录、依赖状态、上层反馈和恢复边界，不能只返回诊断而留下悬挂任务。现有权限、预算、路径和命令确认守卫在所有阶段继续生效，不等待本阶段才启用。
...
4. `add preflight task validation`（暂缓，按具体问题重新评估后再安排）

每次提交都删除被替代的重复路径，不保留旧接口兼容层。当前阶段 0–2 已完成，可独立进入合并评审；阶段 3 暂缓，不阻塞本轮收尾。若后续实施阶段 3，另行完成相应回归与服务器验收。

## 11. 完成标准

当前已实施部分的完成标准如下；阶段 3 若重新启动，应补充其明确的阻断与恢复验收，不以暂缓项否定阶段 1、2 的独立完成：
...
- 已同步现行接口、上下文、架构和 CLI 文档。阶段 3 新的运行前 Validation 暂缓实现，决定与重新评估条件见 §6。
...
验收覆盖明确交付及一次受控缺失后的恢复，不证明重复运行可靠性或科学结论有效性。正常场景还经历了未读引用与非法 verdict 的纠正；HTTP 调用成功或无重试不表示领域动作首次即通过。阶段 2 已独立收尾；阶段 3 实现暂缓，见 §6 的 2026-09-26 决定。
