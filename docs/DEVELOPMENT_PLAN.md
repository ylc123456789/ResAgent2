...
| 2026-09-04 | 共享数据集目录接入 | `feat/dataset-catalog` | 本地与服务器均 501 passed、1 skipped；服务器真实 CLI 自动识别 cifar10、缺 ImageNet-1K 时 paused/ask_user；code→experiment completed/4 artifacts | `DatasetCatalog` 从共享根自动装入/恢复 Run；删除普通 CLI 的逐次路径参数；Scientific/Coding/Experiment 共用缺失资源策略，Coding verification 与 Experiment command 共用数据集环境。code→experiment 仍有既存 delivery warning（期望指标命名/分立日志与实际产物不完全一致），不属于本项资源链路。 |
