...
稳定导入路径是 `resagent2_contracts`。包版本为 `0.1.0`，当前 wire schema 版本为 `7.0`。ResearchRequest 不含部署资源；dataset_refs 只在系统状态和内部调用中传递。UserAnswer 仍是调用方提交的答案；RecordedAnswer 由 Controller 配上已持久化的 question_text，供 Run 和内部调用保存、传递。旧 6.0 及更早 Run 不支持恢复，原件保留、不迁移。
