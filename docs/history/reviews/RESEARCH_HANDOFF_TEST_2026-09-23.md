...
后续状态（2026-09-24）：原公开整链和 CUDA 训练已复核；后续补测补齐 Experiment 公开 CLI 问答恢复、独立 GPU 采样和脚本留存，三项验收缺口关闭，本轮可收尾，无需继续补跑。已知 validation 失败处理及任务答案可发现性问题单独保留，见[独立复核与最终收尾](RESEARCH_HANDOFF_SERVER_REVIEW_2026-09-24.md#verified-closeout)。下文保留原验收要求，不倒改为较宽标准。
