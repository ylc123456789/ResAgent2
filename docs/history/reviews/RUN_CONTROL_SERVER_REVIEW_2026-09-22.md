...
最新结论：修复产品 `602ffee` 已在服务器实测 `8b071e6a` 下通过定向功能复核，两处产品缺陷关闭，未发现新产品缺陷。原轮次失败保留；修复轮的证据缺口、测试方法及计量边界见[第 5 节](#fixed-round)。开发分支已按用户授权合入 main，见第 7 节。
...
2026-09-22 用户明确授权后，更新远端引用，确认工作区干净，将 main 从 `f771a70eef3f3e23c126f720fcd4e2146536e81f` 快进到 `refactor/run-control` 的 `84063e453c5085f63b1185fbb85c44de14dfcbee`。合并无冲突，文件树与已审查开发分支完全相同，统一 IO 及 Run 控制的所有分段提交保留。随后仅补当前合并状态文档，并推送 main。

产品代码仍与 `602ffee` 一致；没有在合并过程中增加实现或重新消耗模型/GPU。开发分支与服务器原始验收记录均保留，服务器主仓库不自动切换，由操作者按交接文档更新 main。schema 13 不恢复旧 schema Run，日常开发与后续测试以新主线为起点。
