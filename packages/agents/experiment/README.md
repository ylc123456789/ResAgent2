...
输入：`ModuleTaskRequest.instruction`（实验及证据语义）、WorkspaceGrant、环境/资源要求，以及结构化 `TaskAcceptanceSpec`。`TaskAcceptanceSpec.required_metric_keys` / `required_artifact_paths` 只用于可信调用方已知的精确 JSON 键和相对路径；LLMCompiler 不猜这些名称，需求留在 instruction。`confirm_before_experiment` 是独立控制字段。
