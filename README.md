...
当前只实现 contracts schema `8.0`（`SCHEMA_VERSION="8.0"`），不保留旧 schema 的第二条运行路径；旧 7.0 及更早的 Run 不支持恢复，既有 state/session/trace 原样保留，不迁移、不重写、不自动清理。Session 的解析边界见 [CONTRACTS](docs/current/CONTRACTS.md#schema)。三个原生 Agent 共用 runtime 和 capabilities，真实执行不依赖旧项目的 Agent。
