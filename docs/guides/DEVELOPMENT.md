...
修改前先按[设计原则与架构约束](../current/DESIGN_PRINCIPLES.md)区分语义问题和执行问题：前者交给职责对应的 LLM，后者用代码检查事实与约束。核对目标向下传递和结果向上反馈是否完整，再检查模块职责、依赖和十二条约束；不能只数 Agent 的 invoke 入口。优先使用已有接口、Tool 和完成检查，同时追踪正常、失败、问答恢复和预算/授权流程。
