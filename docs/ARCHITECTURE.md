...
| WorkspaceSpec | 一个逻辑工作区的来源声明（source_kind + location + environment），不保存物理路径 | ResAgent 的 composition root 声明 |
