...
四组工具最初直接定义在各自 `__init__.py`；第一阶段统一移到同目录的 `tools.py`，随后按模型可调用的 Tool 拆到同目录的独立模块（如 `read_file.py`、`literature_search.py`），`__init__.py` 只显式导出 Tool 与输入模型。实现文件更容易定位，原有公开导入保持可用；不改变上述职责边界，也不引入自动注册。
